...
void display_header(int month, int year) {
    cout << "\033[1;36m" << string(35, '=') << "\n"
         << "       " << get_month_name(month) << " " << year << "\n"
         << string(35, '=') << "\033[0m\n";
}

void display_day_events(const string& date) {
    if (!events_map.count(date)) return;
    
    cout << "\033[1;33m";
    for (const auto& event : events_map[date]) {
        string priority_color;
        switch(event.priority) {
            case 1: priority_color = "\033[1;31m"; break; // Red for high priority
            case 2: priority_color = "\033[1;35m"; break; // Magenta
            case 3: priority_color = "\033[1;34m"; break; // Blue
            case 4: priority_color = "\033[1;32m"; break; // Green
            case 5: priority_color = "\033[1;37m"; break; // White for low priority
        }
...
void display_calendar(int month, int year) {
    ...
    for (const auto& d : days) cout << "\033[1;33m" << setw(5) << d << "\033[0m";
    cout << "\n";
...
void add_event() {
    ...
    while (true) {
        ...
        cin >> e.priority;
        if (e.priority >= 1 && e.priority <= 5) break;
...
void edit_event() {
    ...
    string name;
    getline(cin, name);
    if (!name.empty()) events_map[date][choice].name = name;

    cout << "New priority (" << events_map[date][choice].priority << "): ";
    string prio;
    getline(cin, prio);
    if (!prio.empty()) events_map[date][choice].priority = stoi(prio);
...
int main() {
    ...
    while (true) {
        ...
        switch (tolower(choice)) {
            case 'n': current_month = (current_month % 12) + 1; break;
            case 'p': current_month = (current_month == 1) ? 12 : current_month - 1; break;
            case 'a': add_event(); break;
            case 'e': edit_event(); break;
            case 'q': return 0;
